...
describe('ThreeSixty', () => {
  ...
  beforeEach(() => {
    ...
    canvas2dContextMock = {
      drawImage: jasmine.createSpy('canvas2dContextMock.drawImage')
    }
  ...
  afterEach(() => {
    const threeSixtyWrapperElement = document.querySelector(
      `.${ThreeSixty.CONTAINER_CLASS}`
    )
  ...
  describe('::initialize', () => {
    it('should wrap the canvas', () => {
      const threeSixty = new ThreeSixty(canvasElement, {
        angles: 36,
        anglesPerImage: 9
      })
    ...
    it('should add the configured hotspots to the DOM tree', () => {
      ...
      const hotspotElements = threeSixtyWrapperElement.querySelectorAll(
        `.${ThreeSixty.HOTSPOT_CLASS}`
      )
    ...
    it('should load the first image initially', done => {
      ...
      spyOn(imageLoader, 'load').and.returnValue(
    ...
    it('should show a hotspot when its configured angle gets reached', () => {
      ...
      expect(hotspotElements[1].classList).toContain(
    ...
    it('should hide a hotspot when its configured end angle is reached', () => {
      ...
      expect(hotspotElements[1].classList).not.toContain(
        ThreeSixty.HOTSPOT_ACTIVE_CLASS
      )
    })
  })
  ...
  describe('::preload', () => {
    it('should preload all images', done => {
      ...
      spyOn(imageLoader, 'load').and.returnValue(
        new Promise(resolve => resolve(imageMock))
      )
